function [x_actual, z_actual, z_modeled,x] = sutlej_Zhada_postSpiti_activeSTDS_capture_NOsed_trans(model_name, kf_input, kf_Zhada_input, Spiti_var_file)
%% Input Variables
tic
%close all

mkdir(model_name) %Create directory for model output files
...
%Subsamp is currently set so input vectors are 100 cells in length. This changes when Spiti capture portion of profile is added. Area is
%already in m^2.
...
x_actual = L_subsamp; % Copy of L_subsamp to allow easy plotting with z_actual (same size)
...
%% Code to insert post-Spiti Capture, Sutlej profile downstream of Spiti-Sutlej confluence

% Need to know downstream distance of Spiti confluence
L_Spiti = 378730.2043;
SpitiI = find(x<=(L_Spiti),1,'last');

% Need to remove the nodes downstream of L_Spiti
% find indices for x > L_Spiti. Use this to delete downstream segments from
% x,X,Y,Ah,D_MFT

X = X(1:SpitiI); % Should redefine array as just the segment upstream from the Spiti confluence. Prep for appending downstream segment from Spiti model
Y = Y(1:SpitiI);
Ah = Ah(1:SpitiI);
D_MFT = D_MFT(1:SpitiI);
x = x(1:SpitiI);
z = Z_simple(1:SpitiI);

% Need to also know where the Spiti confluence is in the Spiti-Sutlej
% Reference Frame, so can isolate segment downstream for inclusion in this
% model.
import_downstream_from = 249326; %Downstream distance of Spiti-Sutlej confluence in SpitiSutlej frame

% Need to import the data from the Spiti-Sutlej model
Spiti_Sutlej_X = load(Spiti_var_file,'X'); %Won't have to update
Spiti_Sutlej_X = Spiti_Sutlej_X.X; %Above line imports as a struct, need to have as an array
Spiti_Sutlej_Y = load(Spiti_var_file,'Y'); %Won't have to update
Spiti_Sutlej_Y = Spiti_Sutlej_Y.Y;
Spiti_Sutlej_z = load(Spiti_var_file,'z'); %Won't have to update
Spiti_Sutlej_z = Spiti_Sutlej_z.z;
Spiti_Sutlej_D_MFT = load(Spiti_var_file,'D_MFT'); %Won't have to update
Spiti_Sutlej_D_MFT = Spiti_Sutlej_D_MFT.D_MFT;

Spiti_Sutlej_x = load(Spiti_var_file,'x'); %Will have to update
Spiti_Sutlej_x = Spiti_Sutlej_x.x;
Spiti_Sutlej_Ah = load(Spiti_var_file,'Ah'); %Will have to update by adding Zhada area
Spiti_Sutlej_Ah = Spiti_Sutlej_Ah.Ah;


%%%%%%%%%%Will need to add following later in code (consider moving)
% Spiti_Sutlej_sed_depth = load(Spiti_var_file,'sed_depth');
% Spiti_Sutlej_sed_depth = Spiti_Sutlej_sed_depth.sed_depth;
Spiti_Sutlej_tote = load(Spiti_var_file,'tote');
Spiti_Sutlej_tote = Spiti_Sutlej_tote.tote;
% Spiti_Sutlej_sed_depth = Spiti_Sutlej_sed_depth(Spiti_Sutlej_x > import_downstream_from);
Spiti_Sutlej_tote = Spiti_Sutlej_tote(Spiti_Sutlej_x > import_downstream_from);

% Update Ah
Spiti_Sutlej_Ah = Spiti_Sutlej_Ah(Spiti_Sutlej_x > import_downstream_from) + 2.3e10; %Add in drainage area of Zhada. Ensures consistency with operations later in code where area is removed for pre-capture and then added for post-capture.
Ah = [Ah Spiti_Sutlej_Ah];

% To combine two segments (arrays), use following notation: 
% X = [X_upstream X_downstream];
% Combine to get X, Y, z, D_MFT
Spiti_Sutlej_X = Spiti_Sutlej_X(Spiti_Sutlej_x > import_downstream_from); %Need to truncate before combining to use in calculating offset below
Spiti_Sutlej_Y = Spiti_Sutlej_Y(Spiti_Sutlej_x > import_downstream_from); %Need to truncate before combining to use in calculating offset below
X = [X Spiti_Sutlej_X];
Y = [Y Spiti_Sutlej_Y];
z = [z Spiti_Sutlej_z(Spiti_Sutlej_x > import_downstream_from)];
D_MFT = [D_MFT Spiti_Sutlej_D_MFT(Spiti_Sutlej_x > import_downstream_from)];

% Update x
Spiti_Sutlej_x = Spiti_Sutlej_x(Spiti_Sutlej_x > import_downstream_from); %Truncate Spiti_Sutlej_x to be downstream from Spiti Confluence. Needs to be after assigning other variables since changes the size of Spiti_Sutlej_x
offset = sqrt((X(SpitiI) - Spiti_Sutlej_X(1))^2 + (Y(SpitiI) - Spiti_Sutlej_Y(1))^2); %Find linear distance between the segments
Spiti_Sutlej_x = Spiti_Sutlej_x - Spiti_Sutlej_x(1) + x(end) + offset; % Update the downstream distance of Spiti_Sutlej_x to be relative to the most upstream node of this segment and then add the downstream distance of the upstream segment and the offset to the downstream distance of the downstream segment
x = [x Spiti_Sutlej_x]; %This needs to be defined prior to identifying along-stream divisions and capture location setup
...
% Capture locations
L_Zhada_cap = 368740; % From Baseline Model. BP added, use index to find X, Y, A, Z.
...
X_Zhada_cap = X(Zhada_capI);
Y_Zhada_cap = Y(Zhada_capI);
A_Zhada_cap = Ah(Zhada_capI);

%Define region between captures
Between_capture = (Zhada_capI +1 : SpitiI);
...
OrogenCoreUp = (x>L_LP1 & x<=L_STDS); %Portion of orogen that appears to have linear uplift gradient upstream of STDS.
...
STDS_HangMax = 0.6e-3; %Sets max uplift of the hanging wall of the STDS
STDS_HangMin = 0.3e-3; %Sets min uplift of the hanging wall of the STDS
...
OrogenCoreUpRate = STDS_HangMax - ((STDS_HangMax - ZhadaRate).*(min(OrogenUpD_MFT)-OrogenUpD_MFT)./(min(OrogenUpD_MFT)-max(OrogenUpD_MFT))); 
...
%uplift(LeoPargil) = LeoPargilRate; %Removed to eliminate LP uplift from
%profile
...
% sed_depth((SpitiI+1):end) = Spiti_Sutlej_sed_depth; % Should be the same length, if not need to fix indexing when initialized/imported.
...
tote((SpitiI + 1):end) = Spiti_Sutlej_tote; % Should be the same length, if not need to fix indexing when initialized/imported.
...
%     z(SpitiI) = z(SpitiI + 1); % Pin the bedrock elevation of the node upstream of the Spiti confluence to the bedrock elevation of the downstream node.
%    sed_depth(Zhada_capI)=sed_depth(Zhada_capI - 1);
%     sed_depth(SpitiI) = sed_depth(SpitiI + 1); % Pin the sed_depth of the node upstream of the Spiti confluence to the sed_depth of the downstream node.
    ...
    % Impose rock uplift (only for segment upstream of Spiti confluence)

    z(1:SpitiI) = z(1:SpitiI) + (uplift(1:SpitiI).*dt);

    % Pin z and sed_depth at outlet (Don't think I need this for this code)

    z(end) = 904; %pinning bottom of profile to local baselevel [approx. elevation at Munsiari])
%    sed_depth(end) = 0; %pin sed depth at end to zero
    z(SpitiI + 1) = Z_SpitiCap; % Pin the bedrock elevation of the upstream node of the segment downstream of the Spiti confluence
%    sed_depth(SpitiI + 1) = sed_depth_SpitiCap; % Pin the sed_depth of the upstream node of the segment downstream of the Spiti confluence
...
%     for j=(Zhada_capI + 1):(SpitiI + 1) %Changed to only model segment between Zhada Capture and Spiti Confluence. Include SpitiI + 1, but not sure if that's necessary
% 
...
%     end
    ...
    %% Update variables and quality control (only update for segment upstream of Spiti confluence)
...
%    sed_depth(1:SpitiI)=sed_depth(1:SpitiI)-dz_s(1:SpitiI); 

    % Calculation of bedrock incision over interval dt. Units: (m);
    dz_b(1:SpitiI)=kf(1:SpitiI).*tau_b(1:SpitiI).*dt;
...
%     end
    ...
    if mod(i*dt,50000)==0 || i==1 % Change to adjust plotting frequency.
        ...
        % Pin z and sed_depth at outlet and at Spiti confluence for plotting purposes.

        z(end) = 904; %pinning bottom of profile to local baselevel
%        sed_depth(end) = 0; %pin sed depth at end to zero
%         z(SpitiI) = z(SpitiI + 1);
%         sed_depth(SpitiI) = sed_depth(SpitiI + 1);
        z(SpitiI + 1) = Z_SpitiCap; % Pin the bedrock elevation of the upstream node of the segment downstream of the Spiti confluence
%        sed_depth(SpitiI + 1) = sed_depth_SpitiCap; % Pin the sed_depth of the upstream node of the segment downstream of the Spiti confluence
...
%         cla(subplot6)
        ...
        save(['./' model_name '/' model_name '_n_' num2str(k)],'pct_topo_diff', 'tau_b','uplift', 'Qw', 'W', 'Ah','x','z','kf','Z_Zhada_top','L_Zhada_cap','age_cap_input','i','dz_b','tote','zcap','DZ','dt')
        tlast=dt.*i;
        zlast=z;
%        sdlast=sed_depth;
        topolast = topo;
...
%         savefig(figure2, ['./output/' model_name 'postcap_n_' num2str(k)],'compact')
        save(['./' model_name '/' model_name '_postcap_' num2str(k)], 'tau_b','uplift', 'Qw', 'W', 'Ah','x','z','kf','Z_Zhada_top','L_Zhada_cap','age_cap_input','i','dz_b','tote','zcap','DZ','dt')
%         cla(subplot1)
%         cla(subplot2)
%         cla(subplot3)
%         cla(subplot4)
...
z_modeled = z;
save(['./' model_name '/' model_name '_final_profile.mat'],'-v7.3')
toc
end
