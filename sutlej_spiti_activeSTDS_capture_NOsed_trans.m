function [z_actual, z_modeled,x] = sutlej_spiti_activeSTDS_capture_NOsed_trans(model_name, kf_input)
%% Input Variables
tic
close all

load('SubsampSpitiSutlejPreZhadaInputs_230307.mat');
...
% Major Faults
L_LP1 = 233745.688143; % Upstream extent of Leo Pargil gneiss dome
L_LP2 = 253911.112371; % Downstream extent of Leo Pargil gneiss dome
L_STDS = 299486.296322; % Intersection with STDS
L_MCT1 = 329196.198899; % Most upstream intersection with MCT, near Karcham (downstream from Reckong Peo)
L_Munsiari = 389228.796258; 
L_MCT2 = 416133.774594;
L_MCT3 = 444330.367647; % Most downstream intersection with MCT, near Tundal
...
Spiti_capI = find(x<=(L_Spiti_cap),1,'last'); %Uses 'last' optional argument in 'find'
...
OrogenCoreUp = (x>L_Spiti_cap & x<=L_STDS); %Portion of orogen that appears to have linear uplift gradient upstream of STDS.
...
t_warmup=25e6; % Units: years
age_cap_input = 5e6; % Duration of post-capture model (i.e. age of capture event). Units: years
...
Ah(Spiti_capI+1:end) = Ah(Spiti_capI+1:end)-Ah(Spiti_capI)+Ah(1);
...
ZhadaRate = 0;
SpitiRate = 0.1e-3; %OrogenMin; %0.3e-3;
...
OrogenCoreUpRate = STDS_HangMax - ((STDS_HangMax - ZhadaRate).*(min(OrogenUpD_MFT)-OrogenUpD_MFT)./(min(OrogenUpD_MFT)-2.0189e+05));%Replaced hard-wired value of D_MFT for LP1 in ZhadaSutlej reference frame to ensure consistent uplift field %max(OrogenUpD_MFT))); 
...
% Assume Spiti does not traverse LP dome
uplift(Spiti_Up) = SpitiRate;%OrogenMin; %0.3e-3;
uplift(Spiti_LP) = SpitiRate;%OrogenMin; %LeoPargilRate;
...
%sed_depth(1) = 0; % Set sed_depth at channel head to zero to prevent non-erosion from taking place.
topo=z; %+sed_depth; % 'topo' defines the channel surface elevation. (Units: m)

% Anything else for Spiti profile????
...
for i=1:length(precapt)
    ...
    z(Spiti_capI) = z(Spiti_capI + 1);
%    sed_depth(Spiti_capI)=sed_depth(Spiti_capI - 1);
...
%     end
    ...
    captureA=Ah(Spiti_capI); %Dummy variable to remember capture area

    Ah(Spiti_capI)=0; % Set downstream area to zero (to avoid a negative result in the first iteration)
...
%         colorbar
%         scatter(X_Spiti_cap,Y_Spiti_cap,130,[1 0 0],'linewidth',3.0)
...
%         colorbar
%         scatter(X_Spiti_cap,Y_Spiti_cap,130,'rx','linewidth',3.0)
% 
...
%         scatter(x(Spiti_capI)./1000,z(Spiti_capI)+sed_depth(Spiti_capI),140,'rx','linewidth',3.0)
% 
...
%             ['Capture location = '  num2str(L_Spiti_cap./1000) ' km'],['Time of capture= ' num2str(t_warmup./1000000) ' My']},...
...
%         cla(subplot6)
        ...
        save(['./output/' model_name '_n_' num2str(k)],'pct_topo_diff','D_MFT', 'tau_b','uplift', 'Qw', 'W', 'Ah','x','X','Y','z','kf','L_Spiti_cap','age_cap_input','i','dz_b','tote','zcap','DZ','dt')
        tlast=dt.*i;
        zlast=z;
%        sdlast=sed_depth;
        topolast = topo;
...
%             'String',{model_name,...%['Bedrock erodability K_f= '  num2str(kf)],...%num2str(kf_Zhada) ' to ' num2str(kf_Orogen)],...
%             ['Rock Uplift = ' num2str(min(uplift).*1000) ' to ' num2str(max(uplift).*1000) 'mm/yr']%,...%['URG subsidence= ' num2str(uplift(1).*1000) 'mm/yr'],... %Changed values to make sure code ran (check with previous version of Alps code)
%             ['Capture location = '  num2str(L_Spiti_cap./1000) ' km downstream'],['Time of capture= ' num2str(t_warmup./1000000) ' My']
%             },...
%             'FitHeightToText','on','FontSize',14,'linestyle','none');
%         drawnow
%         print('-dpng',['./moviefiles/' model_name 'postcap_n_' num2str(k)])
%         savefig(figure2, ['./output/' model_name 'postcap_n_' num2str(k)],'compact')
        save(['./output/' model_name '_postcap_' num2str(k)],'D_MFT', 'tau_b','uplift', 'Qw', 'W', 'Ah','x','X','Y','z','kf','L_Spiti_cap','age_cap_input','i','dz_b','tote','zcap','DZ','dt')
%         cla(subplot1)
%         cla(subplot2)
%         cla(subplot3)
%         cla(subplot4)
...
z_modeled = z;
save(['./output/' model_name 'final_spiti_profile.mat'],'-v7.3')
toc
end
