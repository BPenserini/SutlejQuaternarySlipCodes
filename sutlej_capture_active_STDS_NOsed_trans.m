function [z_actual, z_modeled,x] = sutlej_capture_active_STDS_NOsed_trans(model_name, kf_input, kf_Zhada_input)
%% Input Variables
tic
close all
...
%% Uplift set-up
% Uplift field generated assuming discrete change in uplift across the STDS using AHe Exhumation rates. 
% Units: m/yr
...
LeoPargilRate = 0.6e-3; %Thiede et al. 2006, faster exhumation rates from 4-2Ma based on AFT. Do not use, since possibly includes an incision component that is due to capture.
...
%uplift(LeoPargil) = LeoPargilRate; %Removed since river parallels fault
%zone and may not be influenced by dome uplift
uplift(OrogenCoreUp) = OrogenCoreUpRate;
uplift(OrogenCoreUniform) = OrogenMax; %Kept the same as in baseline model
uplift(OrogenCoreDown) = OrogenCoreDownRate; %Kept the same as in baseline model
