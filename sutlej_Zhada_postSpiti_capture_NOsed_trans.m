function [x_actual, z_actual, z_modeled,x] = sutlej_Zhada_postSpiti_capture_NOsed_trans(model_name, kf_input, kf_Zhada_input, Spiti_var_file)
%% Input Variables
tic
%close all

mkdir(model_name); %Make output directory for model output
...
%     sed_depth(SpitiI) = sed_depth(SpitiI + 1); % Pin the sed_depth of the node upstream of the Spiti confluence to the sed_depth of the downstream node.
    ...
    %(SpitiI + 1) = sed_depth_SpitiCap; % Pin the sed_depth of the upstream node of the segment downstream of the Spiti confluence
